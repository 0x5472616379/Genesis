...
public class IndexedFileSystem : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        foreach (var index in _indices) index?.Close();
    }
    ...
    public MemoryStream GetFile(FileDescriptor descriptor)
    {
        ...
        for (var i = 0; i < blocks; i++)
        {
            ...
            if (i != curChunk) throw new InvalidOperationException("Chunk id mismatch.");
            ...
            if (size > read)
            {
                if (nextType != descriptor.GetType() + 1) throw new InvalidOperationException("File type mismatch.");
    ...
    private void DetectLayout(string baseDirectory)
    {
        ...
        if (indexCount <= 0) throw new FileNotFoundException($"No index file(s) present in {baseDirectory}."); 
    ...
    {
        if (type < 0 || type >= _indices.Length) throw new IndexOutOfRangeException("File type out of bounds.");

...
public class ItemDefinition
{
    ...
    public static void Init(ItemDefinition[] definitions)
    {
        ...
        for (var id = 0; id < definitions.Length; id++)
        {
            ...
            // if (def != null && def.Id != -1)
            // {
            //     itemIds.Add(def.Id.ToString());
            // }
            //
            // File.AppendAllLines("ids.txt", itemIds);
    ...
    public string GetGroundAction(int id) => id >= 0 && id < groundActions.Length
        ? groundActions[id]
        : throw new IndexOutOfRangeException("Ground action id is out of bounds.");

    public int GetId() => Id;

    public string GetInventoryAction(int id) => id >= 0 && id < inventoryActions.Length
        ? inventoryActions[id]
